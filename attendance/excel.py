...
def report(year, month, output=None):
    ...
    worksheet.merge_range('A1:J6', 'AJAY KUMAR GARG ENGINEERING COLLEGE, GHAZIABAD \n' +
                          'CENTRAL LIBRARY \n ' +
                          'LIBRARY VISITORS REPORT: ' + date_now.strftime("%B %Y"), merge_format)
    ...
    for heading, x in fst_col:
        worksheet.merge_range(x+'8' + ':' + x + '11'+ '', heading,merge_format)
        if x != 'A' and x != 'B':
    ...
    for day in days:
        ...
        attds_slot_1 = Attendance.objects.filter(entry_time__gt=time_start_slot_1,
                                                 entry_time__lt=time_end_slot_1).all()
        attds_slot_2 = Attendance.objects.filter(entry_time__gt=time_start_slot_2,
                                                 entry_time__lt=time_end_slot_2).all()
        attds_slot_3 = Attendance.objects.filter(entry_time__gt=time_start_slot_3,
                                                 entry_time__lt=time_end_slot_3).all()
        attds_slot_4 = Attendance.objects.filter(entry_time__gt=time_start_slot_4,
        ...
        ls = [attds_slot_1_count, attds_slot_2_count, attds_slot_3_count, attds_slot_4_count,
